...
# Coronavirus (COVID-19) Cases in Egypt
...
df <- read_csv(here::here("/Users/yanhuili/Desktop/yli807-BlogEntry/df.csv"))
```
...
egydf <- read_csv(here::here("/Users/yanhuili/Desktop/yli807-BlogEntry/egydf.csv"))
...
```{r africadf,fig.cap="Covid19 Cases in Middle East & North Africa"}
africacovid <- df %>% 
...
africadf <- africacovid %>% 
  ...
  mutate(gdpCapitaRank = order(order(gdp_capita, decreasing = T))) %>% 
  rename(gdpCapita_2010 = `gdp_capita`) %>% 
  relocate(income, .after = gdpCapitaRank) %>% 
  rename(Income_level = `income`)
...
formattable(africadf, list(
    ...
    gdpCapita_2010 = color_tile("transparent", "#7DCEA0")
    ),
    align = "c")
...
# Coronavirus (COVID-19) Vaccinations in Egypt
...
```{r egyvac, fig.cap="Vaccinations information in Egypt"}
totalvac_egy <- read_csv(here::here("/Users/yanhuili/Desktop/yli807-BlogEntry/cumulative-covid-vaccinations.csv")) %>% 
  filter(Code == "EGY")

onevac_egy <-read_csv(here::here("/Users/yanhuili/Desktop/yli807-BlogEntry/people-vaccinated-covid.csv")) %>% 
  filter(Code == "EGY") %>% 
  select(-`145608-annotations`)

receviedall_egy <- read_csv(here::here("/Users/yanhuili/Desktop/yli807-BlogEntry/share-people-fully-vaccinated-covid.csv")) %>% 
  filter(Code == "EGY") %>% 
...
DT::datatable(egyvac, options = list(pageLength = 10))
```
